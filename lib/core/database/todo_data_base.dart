...
class TodoDataBase {
  ...
  Future<Database> initDataBase() async {
    ...
    Database mydb =
        await openDatabase(pathDataBaseName, version: 1, onCreate: _onCraete);
    return mydb;
  }
...
''');
  ...
  Future<int> updateTodo(int idTodo) async {
    ...
    int status = await database!
        .rawUpdate('UPDATE $todo SET $isDone = ? WHERE $id = ?', [1, idTodo]);
    return status;
  }
  ...
  Future<List> getDoneTodo() async {
    ...
    List status = await database!
        .rawQuery('SELECT * FROM $todo WHERE $isDone = 1  ORDER BY $id DESC');
    return status;
  }
  ...
  Future<List> getNotDoneTodo() async {
    ...
    List status = await database!
        .rawQuery('SELECT * FROM $todo WHERE $isDone = 0  ORDER BY $id DESC');
    return status;
  }
  ...
  Future<int> deleteTodo(int idTodo) async {
    Database? database = await db;
    int status =
        await database!.delete(todo, where: '$id = ?', whereArgs: [idTodo]);
    return status;
  }
